...
class MY_WP_CONTRIBUTIONS {
	...
	public function __construct() {
		$this->get_contributions();
	}

	public function get_curl_body( $url ) {
		$ch = curl_init();
	...
	public function get_contributions() {
		...
		// Core commits
		$get_curl_body = $this->get_curl_body( 'https://core.trac.wordpress.org/search?q=props+' . $username . '&noquickjump=1&changeset=on' );
		$html          = str_get_html( $get_curl_body );
		$mt            = $html->find( 'span[class=numresults]' );
		$string        = explode( ' ', $mt[0]->innertext );
		$core_commits  = (int) rtrim( $string[4], ')' );

		// Meta commits
		//// plugins
		$get_curl_body  = $this->get_curl_body( 'https://plugins.trac.wordpress.org/search?q=props+' . $username . '&noquickjump=1&changeset=on' );
		$html           = str_get_html( $get_curl_body );
		$mt             = $html->find( 'span[class=numresults]' );
		$string         = explode( ' ', $mt[0]->innertext );
		$plugin_commits = (int) rtrim( $string[4], ')' );
		//// meta
		$get_curl_body = $this->get_curl_body( 'https://meta.trac.wordpress.org/search?q=props+' . $username . '&noquickjump=1&changeset=on' );
		$html          = str_get_html( $get_curl_body );
		$mt            = $html->find( 'span[class=numresults]' );
		$string        = explode( ' ', $mt[0]->innertext );
		$meta_commits  = (int) rtrim( $string[4], ')' );

		$total_commits = $core_commits + $plugin_commits + $meta_commits;

		echo '<p>Code commits (props): ' . $total_commits . '</p>' . PHP_EOL;

		// Tickets
		//// core
		echo '<h3>Core tickets currently tracking:</h3>' . PHP_EOL;

		$get_curl_body = $this->get_curl_body( 'https://core.trac.wordpress.org/my-comments?USER=' . $username );
		$html          = str_get_html( $get_curl_body );
		...
		foreach ( $ms as $i => $t ) {
			preg_match( '/href="\/ticket\/(.*)">(.*)<\/a>/', $ms[ $i ], $ticket );
			preg_match( '/class="component">(.*)<\/td>/', $mc[ $i ], $component );
			echo '<p class="ticket">' . $component[1] . ' <a href="https://core.trac.wordpress.org/ticket/' . $ticket[1] . '" target="_blank">#' . $ticket[1] . '</a><br/>&therefore; ' . $ticket[2] . '</p>' . PHP_EOL;
		}

		//// meta
		echo '<h3>Meta tickets currently tracking:</h3>' . PHP_EOL;

		$get_curl_body = $this->get_curl_body( 'https://meta.trac.wordpress.org/report/7?USER=' . $username );
		$html          = str_get_html( $get_curl_body );
		$mt            = $html->find( 'td[class=ticket]' );
		$mc            = $html->find( 'td[class=component]' );
		$ms            = $html->find( 'td[class=summary]' );
		...
		foreach ( $mt as $i => $t ) {
			preg_match( '/href="\/ticket\/(.*)">(.*)<\/a>/', $ms[ $i ], $ticket );
			preg_match( '/class="component">(.*)<\/td>/', $mc[ $i ], $component );
			echo '<p class="ticket">' . $component[1] . ' <a href="https://meta.trac.wordpress.org/ticket/' . $ticket[1] . '" target="_blank">#' . $ticket[1] . '</a><br/>&therefore; ' . $ticket[2] . '</p>' . PHP_EOL;
		}
	}
}

// Initialize My WP Contributions.
MY_WP_CONTRIBUTIONS::get_instance();
